...
class ModifiedUNet3DEncoder(nn.Module):
    ...
    def __init__(self, in_channels=1, base_n_filter=8, attention=False, depth=3):
    ...
    def forward(self, x):
        ...
        out = self.lrelu(out)
        context_features.append(context_1)
        ...
        out = self.lrelu(out)
        context_2 = out
        context_features.append(context_2)
        ...
        out = self.lrelu(out)
        context_3 = out
        context_features.append(context_3)
        ...
        if self.depth == 4:
            ...
            out = self.lrelu(out)
            context_4 = out
            context_features.append(context_4)
...
class ModifiedUNet3DDecoder(nn.Module):
    ...
    def __init__(self, n_classes=1, base_n_filter=8, depth=3):
...
if __name__ == "__main__":
    ...
    # x = torch.randn((4, 1, 192, 192, 32))
    x = torch.randn((4, 1, 44, 96, 160))
    encoder = ModifiedUNet3DEncoder(in_channels=1, base_n_filter=32, depth=4)
    decoder = ModifiedUNet3DDecoder(n_classes=1, base_n_filter=32, depth=4)
