...
class Model(pl.LightningModule):
    def __init__(self, args, center_name, datalists_root, centers_list, optimizer_class, load_pretrained=False, 
    ...
    def prepare_data(self):
        ...
        # NOTE: Training on first center will be same for both Fine-tuning and Replay
        ...
        if self.center_idx == 0:
            ...
            # load the current dataset of the center
            dataset = os.path.join(self.datalists_root, self.args.dataset_type, f"dataset_{self.center_name}.json")
            current_train_files = load_decathlon_datalist(dataset, True, "training")

            # store samples from previous datasets in the buffer
            # NOTE: it goes from 1 to center_idx+1 everytime because the buffer is emptied at the end of the function
            for i in range(1, self.center_idx + 1):
                prev_center_name = self.centers_list[i - 1]
                prev_dataset = os.path.join(self.datalists_root, self.args.dataset_type, f"dataset_{prev_center_name}.json")
                prev_train_files = load_decathlon_datalist(prev_dataset, True, "training")

                if self.args.replay_strategy == 'random':
                    random.shuffle(prev_train_files)
                    
                    # NOTE: some datasets have less samples. Store all of them if less than the
                    # total replay samples else save only a few as defined 
                    if len(prev_train_files) < self.args.num_replay_samples_per_center:
                        self.buffer += prev_train_files
                        logger.info(f"Adding all samples from {prev_center_name} to the memory buffer!")
                    else:
                        self.buffer += prev_train_files[:self.args.num_replay_samples_per_center]
                        logger.info(f"Adding {self.args.num_replay_samples_per_center} samples RANDOMLY from {prev_center_name} to the memory buffer!")
                
                elif self.args.replay_strategy == 'max_entropy_label':
                    # these are the train files that are sorted in the descending order of entropy
                    # NOTE: we don't shuffle here
                    prev_train_files_sorted = utils_data.sort_by_entropy(prev_train_files, descending=True)
                    if len(prev_train_files_sorted) < self.args.num_replay_samples_per_center:
                        self.buffer += prev_train_files_sorted
                        logger.info(f"Adding all samples from {prev_center_name} to the memory buffer in the descending order of entropy!")
                    else:
                        self.buffer += prev_train_files_sorted[:self.args.num_replay_samples_per_center]
                        logger.info(f"Adding {self.args.num_replay_samples_per_center} samples from highest entropy labels from {prev_center_name} to the memory buffer!")
                else:
                    # print and raise an error 
                    logger.error(f"Replay strategy {self.args.replay_strategy} not implemented!")
            
            # add buffer to the current train files        
            train_files = current_train_files + self.buffer
            
            random.shuffle(train_files)

        # print the size of the buffer
        logger.info(f"Memory Buffer Size: {len(self.buffer)}")
        ...
        self.test_datasets_list = utils_data.get_test_datasets(dataset_type=self.args.dataset_type, dataset_path=dataset_pth,
                        test_centers=self.centers_list, datalists_root=self.datalists_root, test_transforms=test_transforms, 
                        seed=self.args.seed)
    ...
    def train_dataloader(self):
        # NOTE: if num_samples=4 in RandWeightedCropd and batch_size=2, then 2 x 4 images are generated for network training
    ...
    def test_step(self, batch, batch_idx, dataloader_idx):
        ...
        if self.args.save_preds:
            ...
            centers_order = "_".join(self.centers_list)
            self.predictions_save_path = os.path.join(self.args.results_dir, self.args.dataset_type, 'ER', centers_order, self.save_exp_id)
        ...
        return {self.centers_list[dataloader_idx]: [test_soft_dice, test_hard_dice, test_precision, test_recall]}
    ...
    def test_epoch_end(self, outputs):
        ...
        for i in range(len(outputs)):
            avg_soft_dice_test[self.centers_list[i]] = (torch.stack([ x[self.centers_list[i]][0] for x in outputs[i] ]).mean()).cpu().numpy()
            avg_hard_dice_test[self.centers_list[i]] = (torch.stack([ x[self.centers_list[i]][1] for x in outputs[i] ]).mean()).cpu().numpy()
            
            avg_precision_test[self.centers_list[i]] = (np.stack([ x[self.centers_list[i]][2] for x in outputs[i] ]).mean())
            avg_recall_test[self.centers_list[i]] = (np.stack([ x[self.centers_list[i]][3] for x in outputs[i] ]).mean())

        logger.info(f"Test (Soft) Dice for centers {self.centers_list}: {avg_soft_dice_test}")
        logger.info(f"Test (Hard) Dice for centers {self.centers_list}: {avg_hard_dice_test}")
        
        logger.info(f"Test Precision Score for centers {self.centers_list}: {avg_precision_test}")
        logger.info(f"Test Recall Score for centers {self.centers_list}: {avg_recall_test}")
...
def main(args):
    ...
    # defining the centers list and order here such that every seed gives a new sequence of centers
    ...
    np.random.shuffle(centers_list)     # randomize the order of centers_list
    ...
    save_path = os.path.join(args.save_path, args.dataset_type, 'ER', centers_order, 
                                f"ER_{args.replay_strategy}_nrs={args.num_replay_samples_per_center}_seed={args.seed}")
    ...
    for i, center in enumerate(centers_list):
        ...
        save_exp_id = f"ER_{center}_{args.replay_strategy}_nrs={args.num_replay_samples_per_center}_seed={args.seed}_{timestamp}"

        if pretrained_load_path is not None:
            pl_model = Model(args, center_name=center, datalists_root=datalists_root, centers_list=centers_list,
                                optimizer_class=optimizer_class, load_pretrained=True, load_path=pretrained_load_path, 
                                exp_id=save_exp_id, replay=True)
        else:
            # i.e. train on the first center by loading weights from scratch
            pl_model = Model(args, center_name=center, datalists_root=datalists_root, centers_list=centers_list, 
                                optimizer_class=optimizer_class, load_pretrained=False, load_path=pretrained_load_path, 
                                exp_id=save_exp_id, replay=False)
        ...
        logger.info("Printing the Best Model Path!")
        # print best checkpoint after training
        print(trainer.checkpoint_callback.best_model_path)

        # load the best checkpoint after training
        loaded_model = pl_model.load_from_checkpoint(trainer.checkpoint_callback.best_model_path, strict=False)
        pretrained_encoder = loaded_model.encoder
        logger.info(f" SAVING THE BEST ENCODER ! ")
        # saving the best encoder whose pre-trained weights will be loaded for the next center
        torch.save(pretrained_encoder.state_dict(), 
                    os.path.join(save_path, f"best_enc_{center}_ER_se={args.seed}.pt"))

        pretrained_load_path = os.path.join(save_path, f"best_enc_{center}_ER_se={args.seed}.pt")
    ...
    with open(os.path.join(save_path, 'test_metrics.txt'), 'a') as f:
        print('\n-------------- Test Metrics from Replay Across all Centers ----------------', file=f)
        print(f"\nSeed Used: {args.seed}", file=f)
        print(f"\nModel: {args.model} \tDepth:{args.unet_depth}", file=f)
        print(f"\nDataset: {args.dataset_type}", file=f)
        print(f"\nNumber of Replay Samples per Center: {args.num_replay_samples_per_center}", file=f)
        print(f"\nReplay Strategy: {args.replay_strategy}", file=f)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-nrs', '--num_replay_samples_per_center', default=20, type=int,
                        help="Number of samples to use per center for rehearsal")
    parser.add_argument('-rep-strat', '--replay_strategy', choices=['random', 'max_entropy_label'],
                        default='random', type=str, help='Replay strategy to use')
