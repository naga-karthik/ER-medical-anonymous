...
class Model(pl.LightningModule):
    ...
    def prepare_data(self):
        ...
        # load the dataset of the center
    ...
    def test_step(self, batch, batch_idx, dataloader_idx):
        ...
        # upon fsleyes visualization, observed that very small values need to be set to zero, but NOT fully binarizing
        # the pred for better visualization
        if self.args.dataset_type == "ms_brain":
        ...
        # binarizing the predictions for calculating the rest of the metrics
        ...
        if self.args.save_preds:
            ...
            self.predictions_save_path = os.path.join(self.args.results_dir, self.args.dataset_type, 'FT', centers_order, self.save_exp_id)
...
def main(args):
    ...
    np.random.shuffle(centers_list)  
    centers_order = "_".join(centers_list)

    # final matrix of test dice scores
    ...
    save_path = os.path.join(args.save_path, args.dataset_type, 'FT', centers_order, f"FT_seed={args.seed}")
    ...
    for i, center in enumerate(centers_list):
        ...
        save_exp_id = f"FT_{center}_seed={args.seed}_{timestamp}"
        ...
        logger.info(f"TRAINED ON CENTER: {center}; TESTING ON ALL CENTERS !!")
        ...
        logger.info("TESTING ON ALL CENTERS DONE !")
        ...
        logger.info("SAVING THE BEST ENCODER !")
        ...
        torch.save(pretrained_encoder.state_dict(), 
                    os.path.join(save_path, f"best_enc_{center}_FT_se={args.seed}.pt"))

        pretrained_load_path = os.path.join(save_path, f"best_enc_{center}_FT_se={args.seed}.pt")

        # closing the current wandb instance so that a new one is created for the next fold
        wandb.finish()


    # with open(os.path.join(args.results_dir, centers_order, f"seed={args.seed}", 'test_dice_matrix.txt'), 'a') as f:
    with open(os.path.join(save_path, 'test_metrics.txt'), 'a') as f:
        print('\n-------------- Test Metrics from Fine-tuning Across all Centers ----------------', file=f)
